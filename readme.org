...
After using [[https://github.com/doomemacs/doomemacs][doom-emacs]] for the whole of 2020, in early 2021 I wrote my
own config from scratch. Over 2 years I accumulated snippets and
packages for a while until it became slow and complex. In 2023 I
decided to simplify my configuration file.

Approach:
- Use [[https://github.com/SystemCrafters/crafted-emacs][crafted-emacs]] as a solid foundation. It provides battle-tested
  =init.el= and =early-init.el=, plus solid defaults so I can focus on
  what really matters, which is my =config.el=.
...
Sections:
- Emacs core settings
  - No external packages, only =(use-package emacs)= blocks
    - Well, only exception is installing =vc-use-package= to add the =:vc= keyword to =use-package=
  - Load modules provided by =crafted-emacs=
- User interface
  - First the =(use-package emacs)= blocks
