...
After using =doom-emacs= for the whole of 2020, in early 2021 I wrote my own config from
scratch. Over 2 years I accumulated snippets and packages for a while until it
became slow and complex. In 2023 I decided to simplify my configuration file.

Approach:
- Use =crafted-emacs= as a solid foundation. It provides battle-tested
  =init.el= and =early-init.el=, it provides solid defaults so I can only
  focus on my =config.el=
